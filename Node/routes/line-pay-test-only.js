...
// 存取`.env`設定檔案使用
import 'dotenv/config.js' //讀取資料庫密碼、API 金鑰
...
const linePayClient = createLinePayClient({
  ...
  channelSecretKey: '701a5f803f12b83a8cfe73314d50350e',
...
const redirectUrls = {
  confirmUrl: 'http://localhost:3001/cart/success',
  cancelUrl: 'http://localhost:3001/cart/cancel',
}
...
router.get('/reserve', async (req, res) => {
  ...
  res.status(200).json({
    ...
    // 你也可以加上其他想回傳給前端的資訊
    success: true,
    message: 'LINE Pay payment URL created successfully.'
  });
    ...
    res.status(500).json({
      success: false,
      message: error.message || 'Failed to reserve LINE Pay payment.'
    });
  }
})
