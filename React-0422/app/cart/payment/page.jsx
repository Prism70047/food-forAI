...
const PaymentForm = () => {
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState('creditCard'); // 'creditCard' 或 'linePay'
  ...
  const handleLinePaySubmit = () => {
    ...
    alert('即將導向 LINE Pay 或顯示 QR Code (這部分需要後端與 LINE Pay API 串接)！');
    // 之後的邏輯：
    ...
    <div className={styles.paymentMethodSelector}>
      <button
        type="button"
          ...
          >
            {/* line.png*/ }
            <img src="/images/cart/line.png" alt="LINE Pay" className={`${styles.paymentIcon} ${styles.linePayIcon}`}/> 
            LINE Pay
          </button>
        </div>

        {selectedPaymentMethod === 'creditCard' && (
          <form onSubmit={handleCreditCardSubmit} className={styles.paymentForm}>
            <h3>信用卡資訊 💳</h3>
            ...
            <div className={styles.formRow}> 
              <div className={`${styles.formGroup} ${styles.expiryDate}`}> {/* 可以組合多個 module class */}
